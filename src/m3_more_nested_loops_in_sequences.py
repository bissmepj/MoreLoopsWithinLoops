...
def run_test_largest_number():
    ...
    expected = 456
    answer = largest_number(([1, 2, 3, 4, 5, 6, 7, 8], [123, 234, 456, 345, 234, 321], []))
    print('Expected and actual are:', expected, answer)
...
def largest_negative_number(seq_seq):
    ...
    for i in range(len(seq_seq)):
        if len(seq_seq[i]) > 0:
            if smallest is None:
                smallest = seq_seq[i][0]
            for j in range(len(seq_seq[i])):
                if smallest > seq_seq[i][j]:
                    smallest = seq_seq[i][j]
    if smallest is None:
        return None
    if smallest < 0:
        return smallest
